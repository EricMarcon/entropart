...
```{r DivEst}
de <- DivEst(q = 1, Paracou618.MC, Simulations = 4)
plot(de)
```
...
```{r DivEstPlot}
de <- DivEst(q = 1, Paracou618.MC, Biased = FALSE, Correction = "Best", Simulations = 4)
summary(de)
plot(de)
```
...
```{r DivProfilePlot}
dp <- DivProfile(seq(0, 2, 0.2), Paracou618.MC, Biased = FALSE, NumberOfSimulations = 4)
summary(dp)
plot(dp)
```
...
```{r EntropyCI}
SimulatedDiversity <- expq(EntropyCI(FUN = Tsallis, Simulations = 4, Ns = PAbd, q = 1), q = 1)
Diversity(PAbd, q = 1)
...
```{r CommunityProfile}
bcProfile <- CommunityProfile(Diversity, PAbd, NumberOfSimulations = 4)
