...
sticker_autoplot <- function(object, ..., ObsColor = "black", H0Color = "red", ShadeColor = "grey75", alpha=0.5, main = NULL, xlab = NULL, ylab = NULL, LegendLabels = c("Observed", "Expected", "Confidence\n enveloppe"))
{
  ...
  thePlot <- ggplot2::ggplot() +
    ggplot2::geom_ribbon(ggplot2::aes(x=r, ymin=lo, ymax=hi, fill=ShadeColor), data=object, alpha=alpha) +
  ...
  thePlot <- thePlot +
    ggplot2::geom_line(ggplot2::aes(x=r, y=value, colour=variable, linetype=variable, size=0.75), data=Lines) +
