...
Diversity measurement can be done through a quite rigorous framework based on entropy, \emph{i.e.}\ the amount of uncertainty calculated from the frequency distribution of a community \citep{Patil1982, Jost2006, Marcon2014a}. 
Tsallis entropy, also known as HCDT entropy \citep{Havrda1967, Daroczy1970, Tsallis1988}, is of particular interest \citep{Jost2006, Marcon2014a} namely because it gathers the number of species, Shannon (\citeyear{Shannon1948}) and Simpson (\citeyear{Simpson1949}) indices of diversity into a single framework.
...
\cite{Marcon2014b} generalized the duality of entropy and diversity, deriving the relation between phylogenetic or functional diversity \citep {Chao2010} and phylogenetic or functional entropy (we will write \emph{phylodiversity} and \emph{phyloentropy} for short), as introduced by \cite {Pavoine2009}. 
...
The \pkg{entropart} package for \proglang{R} \citep{R} enables calculation of all these measures of diversity and entropy and their partitioning.
...
\cite{Marcon2014a} derived the decomposition of Tsallis $\gamma$ entropy into its $\alpha$ and $\beta$ components, generalized to phylodiversity \citep{Marcon2014b} and similarity-based diversity \citep{Marcon2014e}.

Estimators of diversity are biased because of unseen species and also because they are not linear functions of probabilities \citep{Marcon2014a}. 
...
Bias-corrected estimators of phylodiversity have been developed by \cite{Marcon2014b}. 
...
This framework is somehow different from that of \cite{Chao2014a} who define $\alpha$ diversity in another way \citep[see][for a detailed comparison]{Marcon2014b}, such that $\alpha$ entropy is not the average surprise of an assemblage. They also propose a definition of functional diversity \citep{Chiu2014b} based in the information brought by pairs of individuals that is not supported in the package.
...
The number of individuals \code{\$Ns} of a \code{MetaCommunity} is theoretically unknown, since communities are just samples of it.
The total number of individuals is \code{\$N}.
For simplicity, it is set to the total number of individuals of all communities.
If community weights are their number of individuals, \code{\$Ns} is just the sum of the numbers of individuals per species of communities.
Else, \code{\$Ns} may contain non-integer values, respecting the probabilities \code{\$Ps} and summing to \code{\$N}.
...
\code{Paracou618.Functional} is an object of class \code{hclust} containing a functional tree based on leaf, height, stem and seed functional traits \citep{Herault2007, Marcon2014b}. 
...
Correction of estimation bias is used to improve the estimation of entropy despite unobserved species and also mathematical issues \citep{Bonachela2008}. 
...
\Sexpr{round(Diversity(PAbd, q = 2))} 
after bias correction (rather than \Sexpr{round(Diversity(PProba, q = 2))} without it).
It means that a community made of \Sexpr{round(Diversity(Ns = PAbd, q = 2))} equally-frequent species has the same Simpson entropy as the actual one. This is much less than the actual 
\Sexpr{round(Paracou618.MC$Nspecies, 0)} sampled species. Simpson's entropy focuses on dominant species. 
...
$\beta$ entropy, the difference between $\gamma$ and $\alpha$, is the generalized Jensen-Shannon divergence between the species distribution of the meta-community and those of communities \citep{Marcon2014a}:
\begin{equation}
^q\!H_{\beta
...
The estimation of the diversity of a meta-community whose numbers of individuals \code{\$Ns} are not integer values can't be done with most corrections, which do require integers.
...
The Chao-Shen correction can be applied based on this sample coverage and the actual \code{\$Ps} values of the meta-community.
Finally, the ``Best'' correction is the greater of the two values obtained by Chao-Shen and Grassberger \citep{Marcon2014a}.
...
The bootstrap confidence intervals of the values of diversity \citep{Marcon2012, Marcon2014a} are calculated if \code{NumberOfSimulations} is not 0.
...
Phylogenetic or functional diversity generalizes HCDT diversity, considering the distance between species \citep{Marcon2014b}. 
...
The decomposition of the diversity of meta-communities with non integer \code{\$Ns} starts with the estimation of $\gamma$ diversity. The best estimator is found for each slice of the tree.
...
The entropart package allows estimating biodiversity according to the framework based on HCDT entropy, the correction of its estimation-bias \citep{Grassberger1988, Chao2003, Chao2015} and its transformation into equivalent numbers of species \citep{Hill1973, Jost2006, Marcon2014a}. 
Phylogenetic or functional diversity \citep{Marcon2014b} can be estimated, considering phyloentropy as the average species-neutral diversity over slices of a phylogenetic or functional tree \citep{Pavoine2009}. 
